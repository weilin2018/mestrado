...
fatu(j)=0;
fant(j)=0;
fatu(225:275)=99; %######: sinal retangular na parte central da grade
fcin=fatu;
...
%     fren(2:jmax-1)=fatu(2:jmax-1)+ q*(fatu(3:jmax)-2*fatu(2:jmax-1)+fatu(1:jmax-2));
    fren(2:jmax-1)=fatu(2:jmax-1)+(2*q*(fatu(3:jmax) - fant(2:jmax-1) + fatu(1:jmax-2)))/(1+2*q);
    fren((fren<0))=0;   
        if(kplot==freqplot)
        kplot=0;
        XX=num2str(tempo);
        eval(['save -ascii fren_exp',XX,'.dat fren']);
        figure(1)
