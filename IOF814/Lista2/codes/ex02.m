...
latid=23*pi/180;            % latitude (em graus, para rad)
...
% values of 7.0711 is related to a SW wind
...
for n=2:nmax
   ...
   % open boundary condition: elevation given by user
    % elevation of -0.2 meters on south and 0.2m on north boundary
    for j=1:jmax
        eta2(1,j)=(-0.2)*kmare(1,j);
        eta2(2,j)=(-0.2)*kmare(2,j);
        eta2(kmax-1,j)=(0.2)*kmare(kmax-1,j);
        eta2(kmax,j)=(0.2)*kmare(kmax,j);
    end
    for k=1:kmax
        eta2(k,1)=(-0.2)*kmare(k,1);
        eta2(k,2)=(-0.2)*kmare(k,2);
        eta2(k,jmax-1)=(0.2)*kmare(k,jmax-1);
        eta2(k,jmax)=(0.2)*kmare(k,jmax);
    end
