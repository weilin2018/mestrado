...
for n=2:nmax
  ...
  if(contplo==freqplo)
    ...
    plt=zeros(kmax,jmax);
    for ii=1:kmax
        res(ii,:)=fren(jmax*(ii-1)+1:jmax*ii);
    end
    contourf(xgrid,ygrid,res,[concorte:0.01:maxconc]);
    grid;
