...
for ind in indexes:
    ...
    T = np.nanmean(ncin.temp[nstepBegin,:,ind,:],axis=0)
    ...
    # density 2D array
    dens = sw.dens(Splot,Tplot,d)

    contours = np.arange(1022,1028,0.1) # contours

    # begin: 18 isotherm position
    cf1  = axes[axesInd,0].contourf(xgrid,-zgrid,dens,contours,cmap=cmo.cm.dense,extend='both')
    ...
    T = np.nanmean(ncin.temp[nstepFinal,:,ind,:],axis=0)
    Tplot,ndist,ndepth,dist2,sig,depth = oceano.crossSection_optimized(lon,depth,sigma,h1,T,horizResolution=horizResolution,vertResolution=vertResolution,depRef=depRef,ind=ind)

    S = np.nanmean(ncin.salt[nstepFinal,:,ind,:],axis=0)
    Splot,ndist,ndepth,dist2,sig,depth = oceano.crossSection_optimized(lon,depth,sigma,h1,S,horizResolution=horizResolution,vertResolution=vertResolution,depRef=depRef,ind=ind)
    ...
    cf2  = axes[axesInd,1].contourf(xgrid,-zgrid,dens,contours,cmap=cmo.cm.dense,extend='both')
...
for ind in indexes:
    ...
    # final 18 isotherm position
    cf3  = axes[axesInd,2].contourf(xgrid,-zgrid,dens,contours,cmap=cmo.cm.dense,extend='both')
