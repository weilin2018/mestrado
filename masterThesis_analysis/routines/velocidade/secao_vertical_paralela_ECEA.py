...
def create_Structure(figsize,experiment):

    fig,axes = plt.subplots(nrows=3,ncols=2,figsize=figsize)

    axes[0,0].set_title('Paralela - %s'%(experiment.replace('A','C')),fontsize=8)
    axes[0,1].set_title('Paralela - %s'%(experiment.replace('C','A')),fontsize=8)
...
title = u'Seção vertical da componente paralela em Ubatuba (superior), Santos (meio) e \nCananéia (inferior),'\
      + u'no dia 15 de Janeiro em %s (esquerda) e %s (direita)'%(exp.replace('A','C'),exp.replace('C','A'))
