...
for ind in indexes:
    ...
    T = np.nanmean(ncin.u[nstepBegin,:,ind,:],axis=0)
    ...
    cf1  = axes[axesInd,0].contourf(xgrid,-zgrid,Tplot,contours,cmap=cmo.cm.speed,extend='both')
    ...
    T = np.nanmean(ncin.u[nstepFinal,:,ind,:],axis=0)
    Tplot,ndist,ndepth,dist2,sig,depth = oceano.crossSection_optimized(lon,depth,sigma,h1,T,horizResolution=horizResolution,vertResolution=vertResolution,depRef=depRef,ind=ind)

    # final 18 isotherm position
    cf2  = axes[axesInd,1].contourf(xgrid,-zgrid,Tplot,contours,cmap=cmo.cm.speed,extend='both')
...
for ind in indexes:
    ...
    # final 18 isotherm position
    cf3  = axes[axesInd,2].contourf(xgrid,-zgrid,Tplot,contours,cmap=cmo.cm.speed,extend='both')
