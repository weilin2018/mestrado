...
def plot_animation(i,lon_sat,lat_sat,sst_sat,lon_mod,lat_mod,sst_mod,savefig=False):
    ...
    # plot modelling data
    mask = np.where(depth > 100.)
    sst = sst_mod[i,:,:]
    sst[mask] = np.nan
    cb2 = m2.contourf(lon_mod,lat_mod,sst,np.arange(19.,33.,1.),latlon=True,cmap=cmo.cm.thermal,extend='both')
...
for i in np.arange(1,30):
    print('plotting %i'%(i))
    plot_animation(i,lon_sat,lat_sat,sst_sat,lon_mod,lat_mod,sst_mod,savefig=False)
