...
def create_structure_2cols(indexes,exp):
    ...
    for ind in indexes:
        if ind == 99:
            ...
            axes[axesInd,1].set_title('%s - 15 Fevereiro'%(exp),fontsize=8)
        if ind == 28:
            axesInd = 1
        if ind == 19:
            axesInd = 2
...
title = u'Seção vertical de temperatura e, Ubatuba (superior), Santos (meio) e Cananéia (inferior),\n'\
      + u'com a estrutura climatológica à esquerda e a estrutura no dia 15 de Fevereiro em %s\n'% (exp)\
      + u'à direita.'
...
for ind in indexes:
    ...
    S = np.nanmean(temp[nstepBegin,:,ind,:],axis=0)
    ...
    cf1  = axes[axesInd,0].contourf(xgrid,-zgrid,Splot,contours,cmap=cmo.cm.thermal,extend='max')
    cs   = axes[axesInd,0].contour(xgrid,-zgrid,Splot,levels=[18.],colors=('k'),linestyles=('--'))
    ...
    S = np.nanmean(temp[nstepFinal,:,ind,:],axis=0)
    Splot,ndist,ndepth,dist2,sig,depth = oceano.crossSection_optimized(lon,depth,sigma,h1,S,horizResolution=horizResolution,vertResolution=vertResolution,depRef=depRef,ind=ind)

    # final 18 isotherm position
    cf2  = axes[axesInd,1].contourf(xgrid,-zgrid,Splot,contours,cmap=cmo.cm.thermal,extend='max')
    cs   = axes[axesInd,1].contour(xgrid,-zgrid,Splot,levels=[18.],colors=('k'),linestyles=('--'))
