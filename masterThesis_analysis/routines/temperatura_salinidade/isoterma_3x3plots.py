...
for ind in indexes:
    ...
    t    = np.delete(temp,inds,axis=1)
    cs   = axes[axesInd,1].contour(dist,newSig,t,levels=[18.],colors=('w'),linestyles=('--'))
    cf2  = axes[axesInd,1].contourf(dist,newSig,t,np.arange(10,32,0.5),cmap=cmo.cm.thermal,extend='max')
...
for ind in indexes:
    ...
    temp = np.nanmean(ncin.temp[tFinal-6:tFinal+6,:,ind,:],axis=0)
    t    = np.delete(temp,inds,axis=1)
    cs   = axes[axesInd,2].contour(dist,newSig,t,levels=[18.],colors=('w'),linestyles=('--'))
    cf3  = axes[axesInd,2].contourf(dist,newSig,t,np.arange(10,32,0.5),cmap=cmo.cm.thermal,extend='max')
