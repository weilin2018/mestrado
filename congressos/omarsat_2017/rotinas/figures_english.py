...
def criar_figura2(savefig=False):
    ...
    pc = plotar_dado2(fig,ax[0],neConc,item='(c)',horas=False,cbPlot=True)
    ax[0].set_title(u'Experiment V - Northeasterly Wind [5$ m s^{-1}$]')
    # segunda imagem com vento de NE
    pc = plotar_dado2(fig,ax[1],swConc,item='(d)',horas=False,cbPlot=False)
    ...
    plt.suptitle(u"Integrated Dispersion of H$^{3}$ After 40 days simulated", fontsize=25, y=0.78)

    if savefig:
        output = SAVE_DIR + "figura2_dispersao5e6.png"
...
def criar_figura4(savefig=False):
    ...
    plt.suptitle('Surface Current Generated by Wind, Tide and Fluvial Discharge',fontsize=25, y=0.78)
