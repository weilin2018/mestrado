...
%  Foram utilizadas as equações de Miranda et al. (2002) (eqs. 11.88 e 11.89 p. 389)
%  ou Miranda et al. (2012) (eqs. 11.88 e 11.89 p. 397) 
...
% Sem escorregamento no fundo # Equação 11.47 p. 378
...
% Formulação matemática - Cálculo da velocidade na superfície de u(x,0) 
% Com atrito moderado no fundo # Equação 11.64 p. 382

cb1=((0.058*g*h.^2)/(k*uo))*rox;
cb2=(0.395*tau)/(ro*k*uo);

uxoa=cb1+0.63*uf+cb2

% Formulação matemática - perfil vertical de Salinidade - (S=S(x,Z));
% Velocidade com atrito máximo - Equação 11.88 p. 389.
...
% Formulação matemática - perfil vertical de Salinidade - (S=S(x,Z));
% Velocidade com escorregamento de fundo - Equação 11.89 p. 389.

coef1=[(g*h.^2)/(k*uo)*(rox)]

coef2=[1./(ro*k*uo)]

sztaub=Sm+(coef1)*uxoa*(-0.144*Z.^5-0.425*Z.^4+0.5*Z.^2-0.105)+uf*(0.091*Z.^5+0.36*Z.^4-0.057)
...
f2 = plot(sztaub,Z,'k');
set(f2,'linewidth',3);
legend('Baroclínico/u-atrito moderado',3)
xlabel('Salinidade - S(^{o}/{o})','fontsize',14);
ylabel('Profundidade, Z','fontsize',14);
